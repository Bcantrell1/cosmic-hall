					...
					<div className="p-4 border-b">
						<div className="flex items-center justify-between">
							...
							<span className="text-sm text-gray-600">
								{unitProgress?.find(progress => progress.unitId === unit.id)?.progress ?? 0}% Complete
							</span>
						</div>
						<div className="relative w-full h-2.5 bg-gray-100 rounded-full mt-3 overflow-hidden">
							<div
								...
								style={{ width: `${unitProgress?.find(progress => progress.unitId === unit.id)?.progress ?? 0}%` }}
							>
								{unitProgress?.find(progress => progress.unitId === unit.id)?.progress === 100 && (
